...
- Privacy depends on the path. If you UPLOAD a file or use SCREEN CAPTURE, the video never leaves your device — only still frames are sent to be analyzed, then deleted. If you paste a YOUTUBE LINK, we send the link (not the video) to our AI provider, which reads the video from YouTube; that only works for videos already public on YouTube. Either way, nothing is shared with other users.
